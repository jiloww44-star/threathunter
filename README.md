...
| **v4.8** CHRONOLOGY & REPRODUCIBILITY (§67/§68 closure) | **Immutable chronology** (`core/chronology.py` + `GET /investigations/{id}/chronology`): derived on read from the PERMANENT audit trail + link table — never a second history — ordered, kinded, and committed by a recomputed digest so trail edits are visible (direct-SQL tamper test-pinned; derivation note scopes itself honestly). **"Re-run investigation"** (`POST /osint/live/rerun`): replays the exact §68 tuple recorded at ingest (query/source/params/timestamp/parser+source version/authorization/result hash) through the FULL governed path and answers `UNCHANGED` (idempotent no-op) or `CHANGED` (supersede + ObservationChanged — §67 detector as comparator). Replays obey §76 (closed case 403), §80 (retired connector 409), and honesty (seed evidence 422 REPLAY_NOT_AVAILABLE; source-down writes nothing). Cases tab: 🕰 Chronology expansion + ↻ Re-run on every observation — mapping: `design/ChronologyRepro-v4.8.md` |
